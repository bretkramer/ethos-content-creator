...
- The only GitHub Actions workflow (`.github/workflows/notify-obsidian-hub.yml`) notifies an external "Obsidian Hub" documentation system on repo activity — it does not build, test, or deploy the app.
...
## Recent Activity
- Application code shows no changes in well over a year. Every commit across the visible history is a `docs: regenerate living state` cycle — nothing under `src/` moves during this period.
...
- Prior to this steady cadence, an Obsidian Hub notify workflow gets wired into CI as a one-off addition, connecting the repo to an external notes/automation system on repo activity — a process change, not a runtime one.
...
---
_Auto-generated by [obsidian-hub](https://github.com/bretkramer/ethos-obsidian-hub) · 2026-09-10_
